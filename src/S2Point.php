...
class S2Point {
    ...
    public static function minus(S2Point $p1, S2Point $p2): S2Point {
        return self::sub($p1, $p2);
    }

    public static function neg(S2Point $p): S2Point {
    ...
    public static function crossProd(S2Point $p1, S2Point $p2): S2Point {
        return new S2Point(
            $p1->y * $p2->z - $p1->z * $p2->y,
            $p1->z * $p2->x - $p1->x * $p2->z,
            $p1->x * $p2->y - $p1->y * $p2->x
    ...
    public static function add(S2Point $p1, S2Point $p2): S2Point {
        return new S2Point($p1->x + $p2->x, $p1->y + $p2->y, $p1->z + $p2->z);
    }

    public static function sub(S2Point $p1, S2Point $p2): S2Point {
        return new S2Point($p1->x - $p2->x, $p1->y - $p2->y, $p1->z - $p2->z);
    }
    ...
    public static function mul(S2Point $p, $m): S2Point {
        return new S2Point($m * $p->x, $m * $p->y, $m * $p->z);
    }

    public static function div(S2Point $p, $m): S2Point {
        return new S2Point($p->x / $m, $p->y / $m, $p->z / $m);
    }

    /** return a vector orthogonal to this one */
    public function ortho(): S2Point {
    ...
    public function largestAbsComponent(): int {
        $temp = $this->fabs($this);
        if ($temp->x > $temp->y) {
            if ($temp->x > $temp->z) {
                return 0;
    ...
    public static function fabs(S2Point $p): S2Point {
        return new S2Point(abs($p->x), abs($p->y), abs($p->z));
    }

    public static function normalize(S2Point $p): S2Point {
        $norm = $p->norm();
        if ($norm !== 0) {
            $norm = 1.0 / $norm;
        }
    ...
    public function angle(S2Point $va): float {
     ...
     */
    public function aequal(S2Point $that, $margin): bool {
    ...
    public function lessThan(S2Point $vb): bool {
        if ($this->x < $vb->x) {
            return true;
        }
        if ($vb->x < $this->x) {
    ...
    public function compareTo(S2Point $other): int {
    ...
    public function toDegreesString(): string {
